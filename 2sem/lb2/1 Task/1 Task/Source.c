...
Dictionary* FillDictionary(Words* word, int WordCount, int* DictionaryCount)
{
    ...
    for (int i = 0; i < WordCount; i++)
    {
        ...
        {
            if ((dictionary != NULL) && (word[j].Swapped != 1) && (word[i].size != 0 && word[i].count > 1 && (word[i].lettersnum + word[j].lettersnum) > ((word[i].size * word[j].count) + (word[j].size * word[i].count))))
            {
